...
void loop(){
  ...
  currentTime = millis();
  if (previousTime + 5000 < currentTime || thermoValue == 999 || previousTime == 0){
    thermoValue = getTemp();
    previousTime = currentTime;
    BlinkM_fadeToHSB(blinkm_addr, (map(thermoValue, -20, 40, 1, 220) + 210) % 255, 255, 255);
    //Serial.println(thermoValue);
  }
  
  if (thermoValue != 999){
    setDisplayTemp(thermoValue, 'C');
  }
